...
public class GestureDrawline extends View {
	...
	private boolean isAllowCenterPoint = false;//是否允许跳过画线中间的点
	/**
	 * 屏幕的宽度和高度
	 */
	...
	public GestureDrawline(Context context, List<GesturePoint> list, GestureCallBack callBack) {
		super(context);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (currentPoint != null) {
				currentPoint.setPointState(GestureCons.POINT_STATE_SELECTED);
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (pointAt == null || currentPoint.equals(pointAt) || GestureCons.POINT_STATE_SELECTED == pointAt.getPointState()) {
				// 点击移动区域不在圆的区域，或者当前点击的点与当前移动到的点的位置相同，或者当前点击的点处于选中状态
				// 那么以当前的点中心为起点，以手指移动位置为终点画线
				if (GestureUtil.IsShowLine) {
				...
				pointAt.setPointState(GestureCons.POINT_STATE_SELECTED);
				
				
				// 判断是否中间点需要选中
				GesturePoint betweenPoint = getBetweenCheckPoint(currentPoint, pointAt);
				if (isAllowCenterPoint&&betweenPoint != null && GestureCons.POINT_STATE_SELECTED != betweenPoint.getPointState()) {
					// 存在中间点并且没有被选中
					...
					// 设置中间点选中
					betweenPoint.setPointState(GestureCons.POINT_STATE_SELECTED);
					// 赋值当前的point;
					currentPoint = pointAt;
				} else {
			...
			invalidate();
			callBack.onGestureRefresh(passWordSb.toString());
			break;
	...
	final class clearStateRunnable implements Runnable {
		public void run() {
			...
			for (GesturePoint p : list) {
				p.setPointState(GestureCons.POINT_STATE_NORMAL);
			}
			invalidate();
			isDrawEnable = true;
		}
	...
	private void drawErrorPathTip() {
		...
		if (GestureUtil.IsShowLine) {
			for (Pair<GesturePoint, GesturePoint> pair : lineList) {
				pair.first.setPointState(GestureCons.POINT_STATE_WRONG);
				pair.second.setPointState(GestureCons.POINT_STATE_WRONG);
				canvas.drawLine(pair.first.getCenterX(), pair.first.getCenterY(),
						pair.second.getCenterX(), pair.second.getCenterY(), paint);// 画线
			}
		}
	...
	public interface GestureCallBack {
		...
		public abstract void onGestureRefresh(String code);
	}
